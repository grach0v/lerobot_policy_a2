...
int knn(at::Tensor& ref, at::Tensor& query, at::Tensor& idx)
{
  ...
  if (ref.is_cuda()) {
    ...
    auto dist = at::empty({ref_nb * query_nb}, ref.options().dtype(at::kFloat));

    float *dist_dev = dist.data_ptr<float>();

    auto stream = c10::cuda::getCurrentCUDAStream(ref.get_device());

    for (int b = 0; b < batch; b++)
    {
    // knn_device(ref_dev + b * dim * ref_nb, ref_nb, query_dev + b * dim * query_nb, query_nb, dim, k,
    //   dist_dev, idx_dev + b * k * query_nb, stream.stream());
      knn_device(ref_dev + b * dim * ref_nb, ref_nb, query_dev + b * dim * query_nb, query_nb, dim, k,
      dist_dev, idx_dev + b * k * query_nb, stream.stream());
    }
