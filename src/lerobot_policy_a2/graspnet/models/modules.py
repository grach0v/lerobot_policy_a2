...
class ApproachNet(nn.Module):
    ...
    def forward(self, seed_xyz, seed_features, end_points):
        ...
        view_score = (
            features[:, 2 : 2 + self.num_view, :].transpose(1, 2).contiguous()
        )  # (B, num_seed, num_view)
...
class CloudCrop(nn.Module):
    ...
    def __init__(self, nsample, seed_feature_dim, cylinder_radius=0.05, hmin=-0.02, hmax_list=None):
        if hmax_list is None:
