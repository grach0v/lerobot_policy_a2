...
def compute_grasp_loss(end_points, use_template_in_training=True):
    end_points["grasp_top_view_inds"]  # (B, Ns)
    end_points["grasp_top_view_rot"]  # (B, Ns, view_factor, 3, 3)
    objectness_label = end_points["objectness_label"]
    fp2_inds = end_points["fp2_inds"].long()
    ...
    torch.gather(top_view_grasp_angles, 2, target_labels_inds).squeeze(2)  # (B, Ns, D)
    torch.gather(top_view_grasp_depths, 2, target_labels_inds).squeeze(2)  # (B, Ns, D)
    ...
    end_points["stage2_grasp_angle_class_acc/0_degree"] = (
        (grasp_angle_class_pred == target_angles_cls)[loss_mask.bool()].float().mean()
    )
    ...
    grasp_tolerance_loss = huber_loss(
        (grasp_tolerance_pred - target_tolerance) / GRASP_MAX_TOLERANCE, delta=1
    )
