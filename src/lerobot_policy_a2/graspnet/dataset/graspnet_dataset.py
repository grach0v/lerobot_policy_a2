        ...
        for x in tqdm(self.sceneIds, desc="Loading data path and collision labels..."):
            for img_num in range(256):
                self.colorpath.append(
                    os.path.join(root, "scenes", x, camera, "rgb", str(img_num).zfill(4) + ".png")
                )
                self.depthpath.append(
                    os.path.join(root, "scenes", x, camera, "depth", str(img_num).zfill(4) + ".png")
                )
                self.labelpath.append(
                    os.path.join(root, "scenes", x, camera, "label", str(img_num).zfill(4) + ".png")
                )
                self.metapath.append(
                    os.path.join(root, "scenes", x, camera, "meta", str(img_num).zfill(4) + ".mat")
                )
                self.scenename.append(x.strip())
                self.frameid.append(img_num)
            if self.load_label:
                collision_labels = np.load(
                    os.path.join(root, "collision_label", x.strip(), "collision_labels.npz")
                )
                self.collision_labels[x.strip()] = {}
                for i in range(len(collision_labels)):
                    self.collision_labels[x.strip()][i] = collision_labels[f"arr_{i}"]
    ...
    def get_data_label(self, index):
        ...
        for i, obj_idx in enumerate(obj_idxs):
            ...
            idxs = np.random.choice(
                len(points), min(max(int(len(points) / 4), 300), len(points)), replace=False
            )
...
def load_grasp_labels(root):
    ...
    valid_obj_idxs = []
    grasp_labels = {}
    for i, _obj_name in enumerate(tqdm(obj_names, desc="Loading grasping labels...")):
        if i == 18:
            continue
        valid_obj_idxs.append(i + 1)  # here align with label png
        label = np.load(os.path.join(root, "grasp_label", f"{str(i).zfill(3)}_labels.npz"))
        tolerance = np.load(os.path.join(BASE_DIR, "tolerance", f"{str(i).zfill(3)}_tolerance.npy"))
        grasp_labels[i + 1] = (
