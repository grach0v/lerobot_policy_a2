...
parser.add_argument(
    "--pos_ratio_thresh", type=float, default=0.8, help="Threshold of positive neighbor ratio[default: 0.8]"
)
parser.add_argument(
    "--mu_thresh", type=float, default=0.55, help="Threshold of friction coefficient[default: 0.55]"
)
...
def manager(obj_name, pool_size=8):
    # load models
    label_path = f"{obj_name}_labels.npz"
    ...
    pool = []
    process_cnt = 0
    work_list = list(range(len(points)))
    ...
    while len(work_list) > 0:
        for ind, p in enumerate(pool):
            if not p.is_alive():
                ...
                pool.append(p)
                process_cnt += 1
                print(f"{process_cnt}/{len(points)}")
                break
    while len(pool) > 0:
        for ind, p in enumerate(pool):
            if not p.is_alive():
    ...
    np.save(f"{save_path}/{obj_name}_tolerance.npy", saved_tolerance)
...
def worker(obj_name, point_ind, params, tolerance):
    ...
    toc = time.time()
    print(f"{obj_name}: point {point_ind} time", toc - tic)
