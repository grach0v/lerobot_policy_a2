...
parser.add_argument(
    "--bn_decay_step", type=int, default=2, help="Period of BN decay (in epochs) [default: 2]"
)
...
parser.add_argument(
    "--lr_decay_rates", default="0.1,0.1,0.1", help="Decay rates for lr decay [default: 0.1,0.1,0.1]"
)
...
def bn_lbmd(it):
    return max(BN_MOMENTUM_INIT * cfgs.bn_decay_rate ** (int(it / cfgs.bn_decay_step)), BN_MOMENTUM_MAX)
    ...
    for batch_idx, batch_data_label in enumerate(TRAIN_DATALOADER):
        ...
        if (batch_idx + 1) % batch_interval == 0:
            ...
            for key in sorted(stat_dict.keys()):
                ...
                log_string(f"mean {key}: {stat_dict[key] / batch_interval:f}")
                stat_dict[key] = 0
...
def evaluate_one_epoch():
    ...
    for key in sorted(stat_dict.keys()):
        ...
        log_string(f"eval mean {key}: {stat_dict[key] / (float(batch_idx + 1)):f}")
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, cfgs.max_epoch):
        ...
        log_string(f"Current learning rate: {get_current_lr(epoch):f}")
        log_string(f"Current BN decay momentum: {bnm_scheduler.lmbd(bnm_scheduler.last_epoch):f}")
