...
class AttentionPool2d(nn.Module):
    ...
    def forward_v(self, x: torch.Tensor):
        """Forward function for computing value features for dense prediction."""
        _, _, w, h = x.shape
        x = x.flatten(start_dim=2).permute(2, 0, 1)
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)
...
class ModifiedResNet(nn.Module):
    """ResNet with anti-aliasing strided convolutions and QKV attention pooling."""
